    ...
    $.extend({
    	...
    	table: {
            ...
            refreshOptions:function(){
                var pageNumber = $.table.getCurrentPageNumber();
            ...
            getCurrentPageNumber:function(){
                return $("#bootstrap-table").bootstrapTable('getOptions').pageNumber;
        ...
        operate: {
            ...
            // 撤销审批
            removeApproval: function(id, state) {
                $.modal.confirm("确定撤销审批吗？", function() {
                    ...
                    url = $.common.isEmpty(state) ? url : url.replace("{state}", state);
                    // var data = { "ids": id };
                    // $.operate.submit(url, "post", "json", data);
            ...
            editAll: function(){
                ...
                $.modal.confirm("确认要删除选中的" + rows.length + "条数据吗?", function() {
                    var url = $.table._option.editAllUrl;
                    var data = { "ids": rows.join() };
                    $.operate.submit(url, "post", "json", data);
                });
			},
			...
			manageAll: function() {
                var rows = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                if (rows.length == 0) {
                    $.modal.alertWarning("请至少选择一名员工");
                    return;
                }
                var url = $.table._option.manageUrl;
                var ids = rows.join()
				// alert(ids)
                $.modal.open("添加" + $.table._option.modalName, url+"?ids="+ids);
			},
            // 清空信息
            clean: function() {
			...
			addAll: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.createAllUrl : $.table._option.createAllUrl.replace("{id}", id);
                $.modal.open("添加" + $.table._option.modalName, url);
            },
            // 员工修改
            edit: function(id) {
            ...
            app: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.appUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
                ...
                $.modal.open("审批设置" + $.table._option.modalName, url);
            },
            ...
            // editApproval: function(id) {
            //     var url = "/404.html";
            //     if ($.common.isNotEmpty(id)) {
            //         url = $.table._option.approvalUrl.replace("{id}", id);
            //     } else {
            //         var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
            //         if (id.length == 0) {
            //             $.modal.alertWarning("请至少选择一条记录");
        ...
        tree: {
        	...
        	init: function(options) {
        	    ...
        	    $.get(options.url, function(data) {
        			...
        			var treeName = $(options.treeName).val()
